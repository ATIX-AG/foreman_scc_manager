  ...
  module V2
    class SccAccountsControllerTest < ActionController::TestCase
      ...
      test 'should create scc_account' do
        ...
        assert account
        post :create,
          params: { name: account.name, login: account.login, password: account.password,
      ...
      test 'should refuse create scc_account' do
        ...
        assert organization
        post :create,
          params: { name: account.name, password: account.password, login: account.login,
                    organization_id: organization.id }
      ...
      test 'should fail to update scc_account with empty name' do
        account = scc_accounts(:two)
        put :update,
          params: { id: account.id, scc_account: { name: '', sync_date: Time.zone.now, interval: 'weekly' } }
      ...
      test 'should fail to update scc_account with empty login' do
        account = scc_accounts(:two)
        put :update,
          params: { id: account.id, scc_account: { login: '', sync_date: Time.zone.now, interval: 'weekly' } }
      ...
      test 'should fail to update scc_account with empty base_url' do
        account = scc_accounts(:two)
        put :update,
          params: { id: account.id, scc_account: { base_url: '', sync_date: Time.zone.now, interval: 'weekly' } }
