      ...
      Foreman::Plugin.register :foreman_scc_manager do
        ...
        security_block :foreman_scc_manager do
          permission :view_scc_accounts,
            { scc_accounts: %i[show index auto_complete_search],
              'api/v2/scc_accounts': %i[show index auto_complete_search] },
          ...
          permission :view_scc_products,
            { scc_products: %i[index show],
              'api/v2/scc_products': %i[index show] },
            resource_type: 'SccProduct'

          permission :subscribe_scc_products,
            { scc_products: %i[subscribe unsubscribe],
              'api/v2/scc_products': %i[subscribe unsibscribe] },
            resource_type: 'SccProduct'
        end
